...
with st.sidebar:
    ...
    simulation_num_items = st.number_input(
        "Number of Top 'A' Items to Simulate", min_value=1, max_value=100, value=3, step=1,
        help="The simulation validates ordering rules on the top N highest-value (Category A) items."
    )
...
@st.cache_data
def validate_decision_rules(df_raw, df_params, sample_days, lead_time, num_products):
    """
    Simulates inventory decisions for the top N 'A' items and returns the full daily log.
    """
    df_raw['Date'] = pd.to_datetime(df_raw['Date'])
    df_daily_sales = df_raw.groupby(['Date', 'Product ID'])['Units Sold'].sum().reset_index()
    
    a_items = df_params[(df_params['ABC_Category'] == 'A') & (df_params['EOQ'] > 0)]['Product ID'].head(num_products).tolist()
...
if df_raw.empty:
    ...
    with st.spinner("Calculating inventory parameters..."):
        ...
        df_validation_log, df_daily_log = validate_decision_rules(df_raw, df_final, simulation_days, lead_time_days, simulation_num_items)
    ...
    if not df_daily_log.empty:
        ...
        with st.sidebar:
            st.markdown("---")
            st.subheader("4. Visualization Control")
