...
if __name__ == "__main__":
    ...
    # ✅ Extract product-category mapping once
    product_category_map = df_raw[['Product ID', 'Category']].drop_duplicates()
    ...
    for product_id, df_product in df_raw.groupby('Product ID'):
        try:
            ...
            # Attach product and category info
            df_result = test_data.copy()
            df_result['Product ID'] = product_id
            df_result['Category'] = product_category_map.loc[
                product_category_map['Product ID'] == product_id, 'Category'
            ].values[0]
            ...
            # Also add category to metrics
            metrics['Product ID'] = product_id
            metrics['Category'] = df_result['Category'].iloc[0]
    ...
    print("✅ Milestone 2 successfully completed (Category-wise data included!)")
