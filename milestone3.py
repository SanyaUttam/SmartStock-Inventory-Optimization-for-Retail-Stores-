...
INVENTORY_PARAMS = {
    ...
    'LEAD_TIME_DAYS': 7,         
    'LEAD_TIME_STD_DEV': 1.0,    # Standard Deviation of Lead Time (in days) - ASSUMED
    'SERVICE_LEVEL': 0.95,       # Desired probability of not stocking out (95%)
    'DAYS_IN_YEAR': 365,
    'MIN_SAFETY_STOCK': 5,       # Minimum SS units for high-value items
    # Service Level Z-Score (1.645 for 95%)
...
def prepare_inventory_data(df_raw, item_cost_map, df_forecast):
    ...
    df_stats = df_daily_sales.groupby('Product ID')['Units Sold'].agg(
        ADD='mean',                # Historical Average Daily Demand (used as a fallback)
        STD_DEV_D='std',           # Standard Deviation of Daily Demand (retained from history)
...
# --- 6. Decision Rule Validation (Simulation) - CORRECTED LOGIC ---

def validate_decision_rules(df_raw, df_params, sample_days=30):
    """
    Simulates inventory decisions for a sample of 'A' items over the last N days
    of historical data to validate ROP/EOQ rules, ensuring only one order per day.
    """
    ...
    # Get the top 3 'A' items for simulation
    a_items = df_params[(df_params['ABC_Category'] == 'A') & (df_params['EOQ'] > 0)]['Product ID'].head(3).tolist()
    
    # Fallback check: Use P0004, P0005, P0016 for robust testing if list is empty
    ...
    # Determine the simulation period (last N days)
    ...
    for date in sorted(df_sample_sales['Date'].unique()):
        ...
        orders_received_today = [
            log for log in inventory_log 
            if log.get('Decision') == 'ORDERED' and log.get('Arrival_Date') == date
        ]
        
        # Track which items placed an order today (to prevent duplicates)
        order_placed_today = {item_id: False for item_id in a_items} 

        for item_id in a_items:
            # Skip if item not in stock tracking (e.g., if initial stock setup failed for an item)
            if item_id not in stock_levels:
                continue
            ...
            if received_qty > 0:
                ...
                inventory_log.append({
                    ...
                    'Stock_Level_at_Decision': stock_level_before_demand, 'Order_Qty': received_qty,
                    'ROP': rop, 'EOQ': eoq
                })
            ...
            stock_level_before_rop_check = stock_levels[item_id] # Stock after receipt, before demand
            
            # --- STOCK UPDATE 2: FULFILL DEMAND ---
            stock_after_demand = stock_level_before_rop_check - demand_today
            # Stock cannot go below zero in this simulation, though the real world allows backorders
            stock_levels[item_id] = max(0, stock_after_demand) 

            # --- ROP CHECK AND ORDER PLACEMENT ---
            order_decision = "DAILY CHECK" 
            order_qty = 0
            
            # CHECK: ROP is met AND no order has been placed for this item today.
            if stock_after_demand <= rop and not order_placed_today[item_id]:
                order_decision = "ORDERED"
                order_qty = eoq
                arrival_date = date + pd.Timedelta(days=P['LEAD_TIME_DAYS'])
                
                # Mark order as placed for today
                order_placed_today[item_id] = True
                
                # Log the order placed 
                inventory_log.append({
                    'Date': date, 'Product_ID': item_id, 'Decision': order_decision,
                    'Stock_Level_at_Decision': stock_after_demand, # ROP check is after demand
                    'Order_Qty': order_qty, 'Arrival_Date': arrival_date,
                    'ROP': rop, 'EOQ': eoq
                })
                
            # Log the daily status (Used for full log analysis, but filtered out of final report)
            inventory_log.append({
                'Date': date, 'Product_ID': item_id, 'Decision': order_decision,
                'Stock_Level_EOD': stock_levels[item_id], 
                'Demand_Today': demand_today,
                'ROP': rop, 'EOQ': eoq
            })

    # Final Log generation
    df_log = pd.DataFrame(inventory_log).reset_index(drop=True)
    
    # Filter for key events: ORDERS and SHIPMENTS
    final_cols = ['Date', 'Product_ID', 'Decision', 'Stock_Level_at_Decision', 'Order_Qty', 'ROP', 'EOQ']
    df_order_events = df_log[df_log['Decision'].isin(['ORDERED', 'SHIPMENT RECEIVED'])].copy()
    
    return df_order_events[final_cols].sort_values(['Date', 'Product_ID'])
...
if __name__ == "__main__":
    ...
    # Using a robust method to define costs
    item_costs = {
        'P001': 250.00, 'P002': 180.00, 'P003': 100.00,
        'P004': 75.00,  'P005': 60.00,  'P006': 50.00,
        'P007': 30.00,  'P008': 20.00,  'P009': 15.00,
        'P010': 10.00 
    }
    ...
    df_validation_log = validate_decision_rules(df_raw, df_params, sample_days=30)
    ...
    if not df_validation_log.empty:
        ...
        # FIX: Filter for ORDERED events where Order_Qty is not NaN to eliminate duplicates
    ...
    # Save the full optimization table
    df_final.to_csv('inventory_optimization_parameters.csv', index=False)
    print(f"📄 Full optimization parameters saved to: inventory_optimization_parameters.csv (contains all {len(product_list)} products)")